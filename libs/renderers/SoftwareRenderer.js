...
THREE.SoftwareRenderer = function ( parameters ) {
	...
	var viewportXscaleXX, viewportYscaleXX, viewportZscaleXX;
	...
	this.setSize = function ( width, height ) {
		...
		var fixscaleXX = 1 << subpixelBits;

		viewportXscaleXX =  fixscaleXX * canvasWidth  / 2;
		viewportYscaleXX = -fixscaleXX * canvasHeight / 2;
		viewportZscaleXX =             maxZVal      / 2;

		viewportXOffs  =  fixscaleXX * canvasWidth  / 2 + 0.5;
		viewportYOffs  =  fixscaleXX * canvasHeight / 2 + 0.5;
	...
	this.render = function ( scene, camera ) {
		...
		for ( var e = 0, el = elements.length; e < el; e ++ ) {
			...
			if ( element instanceof THREE.RenderableFace ) {
				...
				var scaleXXX = element.scaleXX.x * 0.5;
				var scaleXXY = element.scaleXX.y * 0.5;
	...
	function drawTriangle( v1, v2, v3, uv1, uv2, uv3, shader, face, material ) {
		...
		var x1 = (v1.x * viewportXscaleXX + viewportXOffs) | 0;
		var x2 = (v2.x * viewportXscaleXX + viewportXOffs) | 0;
		var x3 = (v3.x * viewportXscaleXX + viewportXOffs) | 0;

		var y1 = (v1.y * viewportYscaleXX + viewportYOffs) | 0;
		var y2 = (v2.y * viewportYscaleXX + viewportYOffs) | 0;
		var y3 = (v3.y * viewportYscaleXX + viewportYOffs) | 0;

		// Z values (.28 fixed-point)

		var z1 = (v1.z * viewportZscaleXX + viewportZOffs) | 0;
		var z2 = (v2.z * viewportZscaleXX + viewportZOffs) | 0;
		var z3 = (v3.z * viewportZscaleXX + viewportZOffs) | 0;
		...
		var minXfixscaleXX = (minx << subpixelBits);
		var minYfixscaleXX = (miny << subpixelBits);

		var c1 = dy12 * ((minXfixscaleXX) - x1) + dx12 * ((minYfixscaleXX) - y1);
		var c2 = dy23 * ((minXfixscaleXX) - x2) + dx23 * ((minYfixscaleXX) - y2);
		var c3 = dy31 * ((minXfixscaleXX) - x3) + dx31 * ((minYfixscaleXX) - y3);
		...
		var cz = ( z1 + ((minXfixscaleXX) - x1) * dzdx + ((minYfixscaleXX) - y1) * dzdy ) | 0;

		// Z pixel steps

		var fixscaleXX = (1 << subpixelBits);
		dzdx = (dzdx * fixscaleXX) | 0;
		dzdy = (dzdy * fixscaleXX) | 0;
		...
		if ( bHasUV ) {
			...
			cbtu = ( tu1 + (minXfixscaleXX - x1) * dtudx + (minYfixscaleXX - y1) * dtudy );
			cbtv = ( tv1 + (minXfixscaleXX - x1) * dtvdx + (minYfixscaleXX - y1) * dtvdy );

			// UV pixel steps
			dtudx = dtudx * fixscaleXX;
			dtudy = dtudy * fixscaleXX;
			dtvdx = dtvdx * fixscaleXX;
			dtvdy = dtvdy * fixscaleXX;
		}
		...
		if ( bHasNormal ) {
			...
			cbnz = ( nz1 + (minXfixscaleXX - x1) * dnzdx + (minYfixscaleXX - y1) * dnzdy );

			// Normal pixel steps
			dnzdx = (dnzdx * fixscaleXX);
			dnzdy = (dnzdy * fixscaleXX);
		}
