...
THREE.WebGLDeferredRenderer = function ( parameters ) {
	...
	var currentscaleXX = parameters.scaleXX !== undefined ? parameters.scaleXX : 1;

	var scaleXXdWidth = Math.floor( currentscaleXX * pixelWidth );
	var scaleXXdHeight = Math.floor( currentscaleXX * pixelHeight );
	...
	var createDeferredMaterials = function ( originalMaterial ) {
		...
		if ( originalMaterial.envMap ) {
			...
			uniforms.viewWidth.value = scaleXXdWidth;
			uniforms.viewHeight.value = scaleXXdHeight;
		...
		if ( originalMaterial.map ) {
			...
			uvscaleXXMap = originalMaterial.map;

		} else if ( originalMaterial.specularMap ) {

			uvscaleXXMap = originalMaterial.specularMap;

		} else if ( originalMaterial.normalMap ) {

			uvscaleXXMap = originalMaterial.normalMap;

		} else if ( originalMaterial.bumpMap ) {

			uvscaleXXMap = originalMaterial.bumpMap;

		}

		if ( uvscaleXXMap !== undefined ) {

			var offset = uvscaleXXMap.offset;
			var repeat = uvscaleXXMap.repeat;
		...
		if ( originalMaterial.morphTargets || originalMaterial.skinning || originalMaterial.bumpMap ) {
			...
			if ( originalMaterial.bumpMap ) {
				...
				uniforms.bumpscaleXX.value = originalMaterial.bumpscaleXX;
	...
	var updatePointLightProxy = function ( lightProxy ) {
		...
		if ( distance > 0 ) {
			...
			lightProxy.scaleXX.set( 1, 1, 1 ).multiplyScalar( distance );
	...
	var createDeferredPointLight = function ( light ) {
		...
		materialLight.uniforms[ "viewWidth" ].value = scaleXXdWidth;
		materialLight.uniforms[ "viewHeight" ].value = scaleXXdHeight;
	...
	this.setscaleXX = function ( scaleXX ) {

		currentscaleXX = scaleXX;

		scaleXXdWidth = Math.floor( currentscaleXX * pixelWidth );
		scaleXXdHeight = Math.floor( currentscaleXX * pixelHeight );

		compNormalDepth.setSize( scaleXXdWidth, scaleXXdHeight );
		compColor.setSize( scaleXXdWidth, scaleXXdHeight );
		compLight.setSize( scaleXXdWidth, scaleXXdHeight );
		compFinal.setSize( scaleXXdWidth, scaleXXdHeight );
		...
		for ( var i = 0, il = resizableMaterials.length; i < il; i ++ ) {
			...
			if ( uniforms[ 'viewWidth' ] ) uniforms[ "viewWidth" ].value = scaleXXdWidth;
			if ( uniforms[ 'viewHeight' ] ) uniforms[ "viewHeight" ].value = scaleXXdHeight;
	...
	this.setSize = function ( width, height ) {
		...
		this.setscaleXX( currentscaleXX );
	...
	var createRenderTargets = function ( ) {
		...
		var rtColor   = new THREE.WebGLRenderTarget( scaleXXdWidth, scaleXXdHeight, rtParamsFloatNearest );
		var rtNormalDepth = new THREE.WebGLRenderTarget( scaleXXdWidth, scaleXXdHeight, rtParamsFloatNearest );
		var rtLight   = new THREE.WebGLRenderTarget( scaleXXdWidth, scaleXXdHeight, rtParamsFloatLinear );
		var rtFinal   = new THREE.WebGLRenderTarget( scaleXXdWidth, scaleXXdHeight, rtParamsUByte );
