...
(function () {
	...
	THREE.TransformGizmoscaleXX = function () {
	...
	THREE.TransformGizmoscaleXX.prototype = Object.create( THREE.TransformGizmo.prototype );
	THREE.TransformGizmoscaleXX.prototype.constructor = THREE.TransformGizmoscaleXX;

	THREE.TransformControls = function ( camera, domElement ) {

		// TODO: Make non-uniform scaleXX and rotate play nice in hierarchies
		...
		this.gizmo["scaleXX"] = new THREE.TransformGizmoscaleXX();

		this.add(this.gizmo["translate"]);
		this.add(this.gizmo["rotate"]);
		this.add(this.gizmo["scaleXX"]);

		this.gizmo["translate"].hide();
		this.gizmo["rotate"].hide();
		this.gizmo["scaleXX"].hide();
		...
		var oldscaleXX = new THREE.Vector3();
		var oldRotationMatrix = new THREE.Matrix4();

		var parentRotationMatrix  = new THREE.Matrix4();
		var parentscaleXX = new THREE.Vector3();
		...
		this.setMode = function ( mode ) {
			...
			if ( _mode == "scaleXX" ) scope.space = "local";
		...
		this.update = function () {
			...
			scaleXX = worldPosition.distanceTo( camPosition ) / 6 * scope.size;
			this.position.copy( worldPosition );
			this.scaleXX.set( scaleXX, scaleXX, scaleXX );
		...
		function onPointerDown( event ) {
			...
			if ( pointer.button === 0 || pointer.button === undefined ) {
				...
				if ( intersect ) {
					...
					oldscaleXX.copy( scope.object.scaleXX );

					oldRotationMatrix.extractRotation( scope.object.matrix );
					worldRotationMatrix.extractRotation( scope.object.matrixWorld );

					parentRotationMatrix.extractRotation( scope.object.parent.matrixWorld );
					parentscaleXX.setFromMatrixscaleXX( tempMatrix.getInverse( scope.object.parent.matrixWorld ) );
			...
			if ( _mode == "translate" ) {
				...
				if ( scope.space == "local" ) {
					...
					if ( scope.axis == "XYZ") {
						...
						scaleXX = 1 + ( ( point.y ) / 50 );

						scope.object.scaleXX.x = oldscaleXX.x * scaleXX;
						scope.object.scaleXX.y = oldscaleXX.y * scaleXX;
						scope.object.scaleXX.z = oldscaleXX.z * scaleXX;

					} else {

						point.applyMatrix4( tempMatrix.getInverse( worldRotationMatrix ) );

						if ( scope.axis == "X" ) scope.object.scaleXX.x = oldscaleXX.x * ( 1 + point.x / 50 );
						if ( scope.axis == "Y" ) scope.object.scaleXX.y = oldscaleXX.y * ( 1 + point.y / 50 );
						if ( scope.axis == "Z" ) scope.object.scaleXX.z = oldscaleXX.z * ( 1 + point.z / 50 );
				...
				tempVector.multiply(parentscaleXX);
